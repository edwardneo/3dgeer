...
from utils.geometry import project_world_camera
        ...
        try:
            for frame, track_ids in frames.items():
                ...
                start = torch.from_numpy(
                    np.loadtxt(root / 'camera_pose_start' / f'{int(frame):03d}_{cam_id}.txt')
                ).float()
                end = torch.from_numpy(
                    np.loadtxt(root / 'camera_pose' / f'{int(frame):03d}_{cam_id}.txt')
                ).float()
                for track_id in track_ids:
                    ...
                    uv, _, valid = project_world_camera(
                        ...
                        'ftheta',
                        ftheta_parameters=p,
                        shutter_type=p.get('shutter_type', 'GLOBAL'),
                    )
                    if valid.sum() < 4:
                        continue
