...
class PhysicalAICameraData(CameraData):
    ...
    def get_camera2worlds(
        cls, data_path, cam_id, start_timestep, end_timestep, directory='camera_pose'
    ):
        root = Path(data_path)
        ...
        return torch.from_numpy(
            np.stack(
                [
                    anchor @ np.loadtxt(root / directory / f'{t:03d}_{cam_id}.txt')
    ...
    def load_calibrations(self):
        ...
        self.shutter_type = self.ftheta_parameters.get('shutter_type', 'GLOBAL')
        args = self.data_path, self.cam_id, self.start_timestep, self.end_timestep
        self.cam_to_worlds = self.get_camera2worlds(*args)
        self.cam_to_worlds_start = self.get_camera2worlds(*args, directory='camera_pose_start')
        K = torch.eye(3)
        ...
        if self.undistort:
            ...
            # Rectification changes projection, not capture time. A global midpoint
            # pose is an explicit approximation; native rolling shutter uses undistort=False.
            midpoint = []
            for start, end in zip(self.cam_to_worlds_start, self.cam_to_worlds):
                rotation, origin = interpolate_camera_poses(start, end, 0.5)
                pose = torch.eye(4)
                pose[:3, :3] = rotation
                pose[:3, 3] = origin
                midpoint.append(pose)
            self.cam_to_worlds = torch.stack(midpoint)
            self.cam_to_worlds_start = self.cam_to_worlds.clone()
    ...
    def to(self, device):
        super().to(device)
        self.device = device
        self.cam_to_worlds_start = self.cam_to_worlds_start.to(device)
    ...
    def get_image(self, frame_idx):
        ...
        start, end = self.cam_to_worlds_start[frame_idx], self.cam_to_worlds[frame_idx]
        y, x = torch.meshgrid(
            torch.arange(height, device=self.device) + 0.5,
            torch.arange(width, device=self.device) + 0.5,
            indexing='ij',
        )
        fraction = camera_shutter_fraction(
            torch.stack((x, y), -1), width, height, self.shutter_type
        )
        rotation, origins = interpolate_camera_poses(start, end, fraction)
        image['viewdirs'] = F.normalize(torch.einsum('...ij,...j->...i', rotation, rays), dim=-1)
        image['origins'] = origins
    ...
    def project_points(self, world_points, frame_idx):
        pixels, depth, valid = project_world_camera(
            world_points,
            self.cam_to_worlds_start[frame_idx],
