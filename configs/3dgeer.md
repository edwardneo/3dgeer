...
2. In the method's config `.yaml` file, under `trainer.render`, add:
    ...
    For [Waymo](../docs/Waymo.md) and [PhysicalAI](../docs/PhysicalAI.md), native training and recorded-camera evaluation use exposure-start/end poses to model rolling shutter. Keep `CamPose` and `CamPosePerturb` disabled. Viewer and novel-view renders use a global shutter.
...
4. Open the interactive viewer:
    ...
    ```shell
    python tools/viewer.py --ckpt $ckpt_path --port 8080
    ```

    Open `http://localhost:8080` to scrub scene time, toggle Gaussian groups, switch between pinhole, fisheye and calibrated FTheta cameras, and export camera-path videos.
